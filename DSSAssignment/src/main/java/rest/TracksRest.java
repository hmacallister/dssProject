...
public class TracksRest {
	...
	public void deleteTrack(Track track) {
		try {
			...
			if (t != null) {
				List<Playlist> playlists = playlistService.getPlaylistsByUser(t
						.getUser().getLibraryPersistentID());
				...
				for (Playlist p : playlists) {
					...
					if (inPlaylist == true) {
						Playlist updatedPlaylist = new Playlist(p.getId(),
								p.getTitle(), p.getUserFK(), playlistTracks);
